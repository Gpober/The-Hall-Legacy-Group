  ...
  if (!customers || customers.length === 0) {
    return (
      <>
        ...
        />
        <div className="mt-4">
          <Link href="/admin/customers/new" className="btn-gold">+ New Customer</Link>
        </div>
      </>
    );
  }
      ...
      <div className="card max-w-2xl p-6">
        <form action={createJob} className="space-y-4">
          ...
          <div className="flex items-center gap-3 pt-2">
            ...
            <Link href={customer ? `/admin/customers/${customer.id}` : "/admin/jobs"} className="btn-ghost">Cancel</Link>
          </div>
        </form>
      </div>
    </>
