  ...
  return (
    <>
      ...
      <div className="mb-4 flex flex-wrap items-center gap-2">
        <Chip label="All" href="/admin/jobs" active={!activeStage} />
        {JOB_STAGES.map((s) => (
          <Chip key={s} label={JOB_STAGE_LABELS[s]} href={`/admin/jobs?stage=${s}`} active={activeStage === s} />
        ))}
      </div>
                ...
                {jobs.map((job) => (
                  <tr key={job.id} className="hover:bg-zinc-50">
                    <td className="px-5 py-3">
                      <Link href={`/admin/jobs/${job.id}`} className="font-medium text-brand-800 hover:underline">
                        {job.title}
                      </Link>
                      {job.claim_number && (
