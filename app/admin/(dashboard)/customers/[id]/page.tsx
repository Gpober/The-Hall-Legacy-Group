...
export default async function CustomerDetailPage({
  ...
  return (
    <>
      <div className="mb-2">
        <Link href="/admin/customers" className="text-sm font-semibold text-brand-700 hover:underline">
          ← Back to customers
        </Link>
      </div>
      <PageHeader title={fullName(customer.first_name, customer.last_name)} subtitle="Customer account">
        <Link href={`/admin/customers/${customer.id}/edit`} className="btn-ghost">Edit</Link>
        <Link href={`/admin/jobs/new?customer=${customer.id}`} className="btn-gold">+ New Job</Link>
      </PageHeader>
          ...
          <div className="card">
            ...
            {jobs && jobs.length > 0 ? (
              <div className="divide-y divide-zinc-100">
                {(jobs as Job[]).map((job) => (
                  <Link key={job.id} href={`/admin/jobs/${job.id}`} className="flex items-center justify-between px-5 py-3 hover:bg-zinc-50">
