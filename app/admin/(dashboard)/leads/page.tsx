  ...
  return (
    <>
      <PageHeader title="Leads" subtitle="Inbound inspection requests and inquiries.">
        <Link href="/admin/leads/new" className="btn-gold">
          + New Lead
        </Link>
      </PageHeader>

      <div className="mb-4 flex flex-wrap items-center gap-2">
        <FilterChip label="All" href="/admin/leads" active={!activeStatus} />
        {LEAD_STATUSES.map((s) => (
          <FilterChip
            key={s}
        ...
        <form className="ml-auto" action="/admin/leads">
          <input
            type="search"
            name="q"
            defaultValue={q}
                ...
                {(leads as Lead[]).map((lead) => (
                  <tr key={lead.id} className="hover:bg-zinc-50">
                    <td className="px-5 py-3">
                      <Link href={`/admin/leads/${lead.id}`} className="font-medium text-brand-800 hover:underline">
